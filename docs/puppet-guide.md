...
that is created from native `mgmt` code in its mcl language. Instead of
the usual `--puppet`, `--puppet-conf`, and `--lang` for mcl, you need
to use alternative flags to make this work:
...
File[/tmp/mgmt_dir/] -> File[/tmp/mgmt_dir/a]
        |
	V
Noop[puppet_handover] -> ( class important_stuff resources )
                              |
                              V
		Noop[puppet_handback]
			|
